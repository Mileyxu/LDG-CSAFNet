...
#     'zipper']
CLASS_NAMES = ['tile', 'carpet','grid',
    'leather','wood','KolektorSDD','MTDD']
# CLASS_NAMES = [
#               'KolektorSDD','scratch']
# CLASS_NAMES = [ 'scratch'
# ]
...
#         return list(x), list(y), list(mask) resize=472, cropsize=448
class MVTecDataset(Dataset):
    def __init__(self,  args, class_name='bottle', is_train=True,
                 resize=472, cropsize=448):
        ...
        # set transforms
        self.transform_x = T.Compose([T.Resize(resize, Image.LANCZOS),
                                      #T.RandomCrop(cropsize),          # ★ 随机裁剪替代中心裁剪
                                      #T.RandomHorizontalFlip(p=0.5),   # ★ 随机翻转
                                      T.CenterCrop(cropsize),
                                      T.ToTensor(),
                                      T.Normalize(mean=[0.485, 0.456, 0.406],
                                                  std=[0.229, 0.224, 0.225])])
    ...
    def __getitem__(self, idx):
        ...
        # 返回4个值：图像、标签、mask、图像路径
        
        return x, y, mask, image_path
